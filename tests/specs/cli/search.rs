...
fn search_label_filter() {
    let temp = init_temp();
    let id3 = create_issue(&temp, "task", "SearchLabelF Task A");
    create_issue(&temp, "task", "SearchLabelF Task B");
    wk().args(["label", &id3, "urgent"])
        .current_dir(temp.path())
        .assert()
        .success();
...
// =============================================================================
// Phase 6: Filter Expression Tests
// =============================================================================
...
// =============================================================================
// Phase 8: Combined Filter and Limit Tests
// =============================================================================
...
fn search_filters_by_prefix() {
    ...
    // Create issue with different prefix
    let output = wk()
        .args([
            "new",
            "task",
            "PrefixSearch Beta task",
            "--prefix",
            "beta",
            "-o",
            "id",
        ...
        .output()
        .unwrap();
    let _id2 = String::from_utf8_lossy(&output.stdout).trim().to_string();
...
fn search_auto_filters_by_project_prefix() {
    ...
    // Create issue with different prefix
    wk().args(["new", "task", "AutoSearch Other task", "--prefix", "beta"])
        .current_dir(temp.path())
        .assert()
        .success();

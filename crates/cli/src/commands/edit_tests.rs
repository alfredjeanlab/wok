...
fn test_update_title() {
    ...
    let result = run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "title", "Updated title");
    assert!(result.is_ok());
...
fn test_update_description() {
    ...
    let result = run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "description", "New description");
    assert!(result.is_ok());
...
fn test_edit_nonexistent_issue_fails() {
    ...
    let result = run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "nonexistent", "title", "New title");
...
fn test_edit_preserves_status() {
    ...
    run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "title", "Updated").unwrap();
...
fn test_description_too_long() {
    ...
    let result = run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "description", &long_desc);
    assert!(result.is_err());
}
...
fn test_empty_description_allowed() {
    ...
    let result = run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "description", "");
    assert!(result.is_ok());
...
fn test_description_events_logged() {
    ...
    run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "description", "Description").unwrap();
...
fn test_description_preserves_other_fields() {
    ...
    run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "description", "New desc").unwrap();
...
fn test_update_type_invalid() {
    ...
    let result = run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "type", "invalid");
    assert!(result.is_err());
}
...
fn test_update_type_same_no_event() {
    ...
    let result = run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "type", "task");
    assert!(result.is_ok());
...
fn test_update_type_preserves_other_fields() {
    ...
    run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "type", "feature").unwrap();
...
fn test_unknown_attribute_fails() {
    ...
    let result = run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "unknown", "value");
    assert!(result.is_err());
}
...
fn test_attribute_case_insensitive() {
    ...
    // Test uppercase
    let result = run_impl(&ctx.db, &ctx.config, &ctx.work_dir, "test-1", "TITLE", "New title");
    assert!(result.is_ok());

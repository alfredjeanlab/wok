...
fn parse_dash_c_before_subcommand() {
    let cli = Cli::try_parse_from(["wok", "-C", "/tmp", "list"]).unwrap();
...
fn parse_long_directory_flag() {
    let cli = Cli::try_parse_from(["wok", "--directory", "/tmp", "list"]).unwrap();
    assert_eq!(cli.directory, Some("/tmp".to_string()));
}
...
#[test]
fn parse_no_dash_c() {
    let cli = Cli::try_parse_from(["wok", "list"]).unwrap();
...
fn parse_dash_c_after_subcommand() {
    ...
    let cli = Cli::try_parse_from(["wok", "list", "-C", "/tmp"]).unwrap();
    assert_eq!(cli.directory, Some("/tmp".to_string()));
}

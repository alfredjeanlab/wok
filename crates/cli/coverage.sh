...
# Find test binaries (filter to this crate only: wk-* and wkrs-*)
OBJECTS=$(find "$TARGET_DIR/debug/deps" -maxdepth 1 -type f -perm +111 \( -name "wk-*" -o -name "wkrs-*" \) ! -name "*.d" ! -name "*.so" | xargs -I{} echo "--object={}")

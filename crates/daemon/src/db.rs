    ...
    pub fn execute_query(&self, op: QueryOp) -> Result<QueryResult, String> {
        match op {
            ...
            QueryOp::ListIssues {
                ...
                let core_status = status.map(|s| s.into());
                let issues = self
                    .core
                    .list_issues(core_status, issue_type, label.as_deref())
                    .map_err(|e| e.to_string())?;
    ...
    pub fn execute_mutate(&self, op: MutateOp) -> Result<MutateResult, String> {
        match op {
            ...
            MutateOp::AddNote {
                ...
                self.core
                    .add_note(&id, status.into(), &content)
                    .map_err(|e| e.to_string())?;
                Ok(MutateResult::Ok)
            }
            MutateOp::LogEvent { event } => {
                let core_event: wk_core::Event = event.into();
                self.core
                    .log_event(&core_event)
                    .map_err(|e| e.to_string())?;
                Ok(MutateResult::Ok)
            }
            MutateOp::AddDependency {
                ...
                self.core
                    .add_dependency(&from_id, &to_id, relation.into())
                    .map_err(|e| e.to_string())?;
                Ok(MutateResult::Ok)
            }
            MutateOp::RemoveDependency {
                ...
                self.core
                    .remove_dependency(&from_id, &to_id, relation.into())
                    .map_err(|e| e.to_string())?;
                Ok(MutateResult::Ok)
            }
            MutateOp::AddLink {
                ...
                let core_link = wk_core::Link {
                    id: 0,
                    issue_id: id,
                    link_type: link_type.map(|t| t.into()),
                    url,
                    external_id,
                    rel: rel.map(|r| r.into()),
                    created_at: Utc::now(),
                };
